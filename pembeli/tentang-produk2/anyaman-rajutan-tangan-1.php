...
$nama_produk = 'Anyaman Rajutan Tangan 1';
...
<head>
    ...
    <title>Anyaman Rajutan Tangan (1) - Detail Produk</title>
    <style>
        body {
...
<body>
    <div class="container">
    <?php
    // Path gambar dari database, fallback ke default jika kosong
    $gambar_produk = !empty($produk['path_gambar']) ? '../' . $produk['path_gambar'] : '../img/no-image.png';
    ?>
    <img class="product-img" src="<?= htmlspecialchars($gambar_produk) ?>" alt="Anyaman Rajutan Tangan 1">
        <div class="product-detail">
            <div class="product-title"><?= htmlspecialchars($produk['nama'] ?? 'Anyaman Rajutan Tangan 1') ?></div>
            <div class="product-info">
                <?= htmlspecialchars($produk['deskripsi'] ?? 'Kerajinan tangan unik, cocok untuk dekorasi rumah.') ?>
            </div>
            ...
            <div class="btn-container">
                ...
                <form method="post" action="./keranjang./tambah-keranjang.php" style="display:inline;">
                    ...
                    <input type="hidden" name="nama_produk" value="<?= htmlspecialchars($produk['nama'] ?? 'Anyaman Rajutan Tangan 1') ?>">
                    ...
                    <input type="hidden" name="file_produk" value="anyaman-rajutan-tangan-1.php">
                ...
                <!-- Beli Langsung ke Checkout -->
                <form method="post" action="./checkout/checkout.php" style="display:inline;">
                    <input type="hidden" name="id_produk" value="<?= htmlspecialchars($produk['slug'] ?? 'anyaman-rajutan-tangan-1') ?>">
                    <input type="hidden" name="nama_produk" value="<?= htmlspecialchars($produk['nama'] ?? 'Anyaman Rajutan Tangan 1') ?>">
                    <input type="hidden" name="harga" value="<?= $produk['harga'] ?>">
                    <input type="hidden" name="jumlah" id="input-jumlah-barang-checkout" value="1">
                    <input type="hidden" name="gambar" value="<?= htmlspecialchars($produk['path_gambar'] ?? 'Anyaman Rajutan tangan 1.jpeg') ?>">
                    <input type="hidden" name="file_produk" value="anyaman-rajutan-tangan-1.php">
