...
<body>
    ...
    ?>
    <img class="product-img" src="<?= htmlspecialchars($gambar_produk) ?>" alt="Basreng 2">
        <div class="product-detail">
            <div class="product-title"><?= htmlspecialchars($produk['nama'] ?? 'Basreng 2') ?></div>
            <div class="product-info">
                <?= htmlspecialchars($produk['deskripsi'] ?? 'Basreng (Bakso Goreng) varian pedas, renyah dan gurih, cocok untuk camilan kapan saja.') ?>
            </div>
            ...
            <div class="btn-container">
                <form method="post" action="./keranjang/tambah-keranjang.php" style="display:inline;" onsubmit="return syncJumlahKeranjang()">
                    <input type="hidden" name="id_produk" value="<?= (int)$produk['id'] ?>">
                    <input type="hidden" name="nama_produk" value="<?= htmlspecialchars($produk['nama'] ?? 'Basreng 2') ?>">
                ...
                <form method="post" action="./checkout/checkout.php" style="display:inline;">
                    <input type="hidden" name="id_produk" value="<?= htmlspecialchars($produk['slug'] ?? 'basreng-2') ?>">
                    <input type="hidden" name="nama_produk" value="<?= htmlspecialchars($produk['nama'] ?? 'Basreng 2') ?>">
                    <input type="hidden" name="harga" value="<?= $produk['harga'] ?>">
    ...
    <script>
        ...
        // Pastikan jumlah yang dikirim ke keranjang sesuai input
        function syncJumlahKeranjang() {
            var input = document.getElementById('jumlah-barang');
            var inputHidden = document.getElementById('input-jumlah-barang');
