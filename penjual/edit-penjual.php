...
$result = $conn->query("SELECT * FROM pengguna WHERE id = $id AND peran = 'penjual'");
...
if (isset($_POST['submit'])) {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        // Cek apakah email sudah dipakai penjual lain
        ...
        if ($cek->num_rows > 0) {
            ...
            // Update data
            $stmt = $conn->prepare("UPDATE pengguna SET nama = ?, email = ?, disetujui = ?, diperbarui_pada = NOW() WHERE id = ? AND peran = 'penjual'");
...
<body>
    ...
    <div class="container my-4">
        <h2 class="section-title">Edit Penjual</h2>
        ...
        <div class="card p-4">
            ...
            <form method="POST">
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="nama" name="nama" required value="<?= isset($_POST['nama']) ? htmlspecialchars($_POST['nama']) : htmlspecialchars($penjual['nama']) ?>">
                </div>
                <div class="mb-3">
                    <label for="email" class="form-label">Email</label>
                    <input type="email" class="form-control" id="email" name="email" required value="<?= isset($_POST['email']) ? htmlspecialchars($_POST['email']) : htmlspecialchars($penjual['email']) ?>">
                </div>
                <div class="mb-3 form-check">
                    <input type="checkbox" class="form-check-input" id="disetujui" name="disetujui" <?= (isset($_POST['disetujui']) ? 'checked' : ($penjual['disetujui'] ? 'checked' : '')) ?>>
                ...
                <a href="penjual.php" class="btn btn-secondary"><i class="bi bi-arrow-left"></i> Kembali</a>
            </form>
        </div>
    </div>
